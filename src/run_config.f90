...
&                  lloc, vkb, evkb, srel, use_dielectric)
...
!Input variables
   ...
   logical :: srel
   logical :: use_dielectric

!Output variables  only printing

!Local variables
   integer :: ii, it, kk, l1, ierr, mch, nvt, i2
...
!real(dp) :: eat(30,2),fat(30,2),rpk(30),eatp(30),fatp(30,2)
   ...
   real(dp), allocatable :: uua(:,:), uu2dv(:)
   real(dp), parameter :: eps_infinity = 80.0d0
   real(dp) :: eps_vacuum(mmax)
   real(dp) :: eps_dielectric(mmax)
   real(dp) :: eps_prefactor
   real(dp) :: eps_tmp(mmax)
   real(dp), allocatable :: v_old(:), v_new(:), dv(:)
   real(dp) :: de, e_old(30)
   integer :: i_eps
   integer, parameter :: n_eps_iter = 8
   real(dp), allocatable :: vpuns_screened(:, :)
   real(dp) :: z_psp
   ...
   interface
      function dielectric(rho, eps_infinity)
         ...
         real(dp), allocatable :: dielectric(:), rho(:)
   ...
   interface
      function spherically_integrate(mmax, rr, obj)
         ...
         real(dp) :: obj(mmax), rr(mmax)
      end function spherically_integrate
   end interface

   allocate (rho(mmax), rhoc(mmax), vi(mmax), vfull(mmax), rhocps(mmax))
   allocate (uu(mmax), up(mmax))
   allocate(vpuns_screened(mmax, 5))
...
!all-electron atom solution for excited state
  ...
  &            rr, vfull, zz, mmax, iexc, eaetst, ierr, srel)
   ...
   if (use_dielectric) then
      eps_dielectric = dielectric(rho, eps_infinity)
   end if
...
!second pseudoatom run from maximally-ionized to excited configuration
   ...
   write (*, '(a,3f16.8)') 'psatom(2) eatp = ', eatp(1:3)

   call psatom(natp, latp, eatp, fatp, nvt, it, rhocps, rho, &
  &           rr, rcmax, mmax, mxprj, iexc, etsttot, nproj, vpuns, lloc, &
  &           vkb, evkb, ierr, eps_vacuum, jj /= 1, uua, .false.)
   ...
   ! dielectric calculation
   if (use_dielectric) then
      allocate(uu2dv(mmax), v_old(mmax), v_new(mmax), dv(mmax))
      do kk = 1, nvt
         eatp(kk) = eat(nc + kk, 3)
         fatp(kk, 1) = fat(nc + kk, 3)
         fatp(kk, 2) = fat(nc + kk, 3)
      end do


      eps_tmp(:) = 1.0d0
      do i_eps = 1, n_eps_iter
         ! Slowly ramp up the dielectric

         do ii = 1, mmax
            eps_tmp(ii) = float(i_eps) / float(n_eps_iter) * (eps_dielectric(ii) - eps_vacuum(ii)) + eps_vacuum(ii)
         end do

         ! Screen vpuns
         z_psp = 0.0_dp
         do ii = 1, nvt
            z_psp = z_psp + fatp(ii, 1)
         end do
         vpuns_screened(:,:) = vpuns(:,:)
         do kk = 1, 5
            call screen_potential_via_gaussian_charge(mmax, rr, z_psp, eps_tmp, vpuns_screened(:, kk))
         end do

         call psatom(natp, latp, eatp, fatp, nvt, it, rhocps, rho, &
  &                 rr, rcmax, mmax, mxprj, iexc, etsttot, nproj, vpuns_screened, lloc, &
  &                 vkb, evkb, ierr, eps_tmp, jj /= 1, uua, jj /= 1)

         if (ierr /= 0) then
            write (6, '(a,a,i2/a,i2)') 'run_config: WARNING for fully non-local PS atom,', &
          &       ' stg. ', 2 + i_eps, &
          &       ' WARNING no output for configuration', jj
            deallocate (rho, rhoc, rhocps, vi, vfull, uu2dv, uua, v_new, v_old, dv)
         end if
      end do
      deallocate(uu2dv, v_new, v_old, dv)
   end if

   if (.not. use_dielectric) then
