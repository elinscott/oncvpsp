...
!
subroutine robust_lschfb(nn, ll, ierr, ee, rr, vv, uu, up, zz, mmax, mch, srel)
...
! relativistic - non-relativistic switch
  ...
  & (ll + 1)*dsqrt((ll + 1)**2 - fss*zz**2))/(2*ll + 1)
   ...
   e_upper = vv(mmax) + 0.5d0*sls/rr(mmax)**2
   e_lower = e_upper
   do ii = 1, mmax
      if (ll == 0) then
! ad-hoc step to eliminate problems from absurd e_lower for ll=0
         e_lower = dmin1(e_lower, vv(ii) + 0.25d0/rr(ii)**2)
      else
         e_lower = dmin1(e_lower, vv(ii) + 0.5d0*sls/rr(ii)**2)
      end if
   end do
   if (ee == 0.0d0) ee = 0.5d0*(e_upper + e_lower)
   if (ee < e_lower) ee = 0.5d0*(e_upper + e_lower)
   if (ee > e_upper) ee = 0.5d0*(e_upper + e_lower)
